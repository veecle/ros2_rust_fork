...
fn message_structure_is_accurate() {
    ...
    let basic_types_fields = vec![
        ...
        ("char_value", BaseType::Char, ValueKind::Simple),
    ...
    let unbounded_sequences_fields = vec![
        ...
        ("char_values", BaseType::Char, ValueKind::Sequence),
        ...
        ("char_values_default", BaseType::Char, ValueKind::Sequence),
        (
            "float32_values_default",
            BaseType::Float,
            ValueKind::Sequence,
    ...
    let wstrings_fields = vec![
        ("string_value", BaseType::WString, ValueKind::Simple),
        (
    ...
    message_structures_and_fields.push(("Strings", wstrings_structure.clone(), wstrings_fields));

    // --------------------- Running the tests ---------------------

    for (message_name, structure, fields) in message_structures_and_fields {
        println!("Testing {}", message_name);
        assert_eq!(
            structure.fields_inorder(),
            fields
                .iter()
        ...
        for (field_name, expected_base_type, expected_value_kind) in fields {
            let field_type = &structure[field_name];
            assert_eq!(field_type.base_type, expected_base_type);
            assert_eq!(field_type.value_kind(), expected_value_kind);
        }
    }
}
...
fn dynamic_message_has_defaults() {
    ...
    assert_eq!(
        ...
        Some(Value::Simple(SimpleValue::Char(&100u8)))
    );
    assert_eq!(
