...
impl<'msg, T> InnerSequence<T> for ProxySequence<'msg, T> where T: PartialEq + ProxyMut<'msg> {
    ...
    fn resize_unchecked(&mut self, resize_function: ResizeFunction, len: usize) {
        let is_ok = unsafe { resize_function(self.sequence as *mut _ as *mut libc::c_void, len) };
        assert!(is_ok);
...
impl<'msg, T> InnerSequence<T> for &'msg mut Sequence<T> where T: PartialEq + SequenceAlloc{
    ...
    /// This will fini all messages in the sequence and re-initialize it from scratch.
    fn resize_unchecked(&mut self, resize_function: ResizeFunction, len: usize) {
        let is_ok = unsafe { resize_function(self as *mut _ as *mut libc::c_void, len) };
        assert!(is_ok);
    }
}
...
#[repr(C)]
pub(super) struct TypeErasedSequence {
    pub(super) data: *mut libc::c_void,
    pub(super) size: libc::size_t,
    pub(super) capacity: libc::size_t,
}
...
pub(super) type ResizeFunction = unsafe extern "C" fn(arg1: *mut libc::c_void, size: usize) -> bool;
...
// impl<'msg, T: SequenceAlloc> Extend<T> for DynamicBoundedSequenceMut<'msg, T> {
//     fn extend<I>(&mut self, iter: I)
//     where
//         I: IntoIterator<Item = T>,
//     {
//         self.inner
//             .extend(iter.into_iter().take(self.upper_bound - self.inner.len()));
//     }
// }

// impl<'msg, T: SequenceAlloc> DynamicBoundedSequenceMut<'msg, T> {
//     /// See [`Sequence::as_slice()`][1].
//     ///
//     /// [1]: rosidl_runtime_rs::Sequence::as_slice
//     pub fn as_slice(&self) -> &[T] {
...
//         // This will drop the previous value of self.inner
//         *self.inner = Sequence::default();
//     }

//     /// Tries to assign a vector to this sequence.
//     ///
//     /// This is only successful if the vector's length is less than the [upper bound][1], otherwise
//     /// the sequence is unmodified.
//     /// The return value indicates whether the operation was successful or not.
//     ///
//     /// [1]: Self::upper_bound
//     pub fn try_assign(&mut self, v: Vec<T>) -> Result<(), SequenceExceedsBoundsError> {
//         if v.len() > self.upper_bound {
//             Err(SequenceExceedsBoundsError {
//                 len: v.len(),
//                 upper_bound: self.upper_bound,
//             })
...
//     /// Tries to reset this sequence to a new sequence of `len` elements with default values.
//     ///
//     /// This is only successful if `len` is less than the [upper bound][1], otherwise
//     /// the sequence is unmodified.
//     /// The return value indicates whether the operation was successful or not.
//     ///
//     /// [1]: Self::upper_bound
//     pub fn try_reset(&mut self, len: usize) -> bool {
//         if len <= self.upper_bound {
//             *self.inner = Sequence::new(len);
//             true
//         } else {
//             false
//         }
...
// impl<'msg, T: Clone + SequenceAlloc> DynamicBoundedSequenceMut<'msg, T> {
//     /// Tries to assign a slice to this sequence.
//     ///
//     /// This is only successful if the slice length is less than the [upper bound][1], otherwise
//     /// the sequence is unmodified.
//     /// The return value indicates whether the operation was successful or not.
//     ///
//     /// [1]: Self::upper_bound
//     pub fn try_copy_from_slice(&mut self, slice: &[T]) -> Result<(), SequenceExceedsBoundsError> {
//         if slice.len() > self.upper_bound {
//             Err(SequenceExceedsBoundsError {
//                 len: slice.len(),
//                 upper_bound: self.upper_bound,
//             })
//         } else {
//             *self.inner = Sequence::new(slice.len());
//             self.inner.clone_from_slice(slice);
//             Ok(())
//         }
//     }
// }
